...
*/	}
    ...
    */
    ...
    private function findDtdInString($string)
    {
        ...
        // add number of matches to result object
        //$this->result->setResult("number_of_doctypes_found",count($matches[0]));
    ...
    */
    ...
    private function compareToExisting($dtd)
    {
        ...
        foreach ($this->doctypes as $name => $doctype) {
...
/*            var_dump(array_udiff_assoc($check,$doctype,function ($a,$b) {

                if($a===$b) {
                    return 0;
                } elseif(str_ireplace("http:","https:",$a)===$b) {
                    return 0;
                } else {
                    return 1;
                }
...
//var_dump($this->result);
            ...
            if(count($diff)===0){
                ...
                //var_dump($this->result->isValid());die();

            }
        }
        return $match;

        //echo "Match was: ".$match;
/*        if(!isset($dtd["si"])) {
        ...
        foreach ($this->doctypes as $name => $doctype) {
            // firstly get the matching root element
            if($doctype["root_element"] === $dtd["root_element"])
            {
                if(!is_null($doctype["kind"]))

                var_dump($name);
                var_dump($doctype);
            }
            if($doctype["si_optional"] === false) {
                // make sure si is present
            }
        }*/
    }
...
/*	public static function autoloader($class) {
		// Check that the class starts with "Doctype"
		if (strpos($class, 'Doctype') !== 0) {
			return;
		}
...
/*	public static function register_autoloader() {
		spl_autoload_register(array('\Beutnagel\Doctype_Validator', 'autoloader'));
	}
*/
